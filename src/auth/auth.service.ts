...
export class AuthService {
  ...
  async forgetPassword(forgetPasswordDto: ForgetPasswordDto) {
    const verifiedKey = `otp_verified:${forgetPasswordDto.email}`;
    const isVerified = await this.redis.get(verifiedKey);

    if (!isVerified) {
      throw new UnauthorizedException('OTP not verified or session expired');
    }
    const result = await this.userService.changePassword(forgetPasswordDto);
  ...
  async verifyOtp(email: string, submittedOtp: string) {
    ...
    await this.redis.set(verifiedKey, 'true', 'EX', 600);
